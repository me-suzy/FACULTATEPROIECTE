...
BEGIN
  ...
  IF pachet_salarizare.v_declansator_PONTAJE = FALSE THEN 
    IF INSERTING THEN 
      RAISE_APPLICATION_ERROR (-20332, 'In SPORURI nu se pot insera inregistrari interactiv !!!') ;
    END IF ;   
    IF UPDATING THEN 
  ...
  -- se verifica si daca stergerea provine din tabela PERSONAL (DELETE CASCADE)
  IF DELETING AND pachet_salarizare.v_regula_del_personal <> 'C' THEN -- grija la variabila publica !!!
    BEGIN 
